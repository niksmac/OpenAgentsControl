...
# or
gh issue comment 123 --repo niksmac/OpenAgentsControl --body "Great idea!"
```
...
# or
gh issue close 123 --repo niksmac/OpenAgentsControl
```
...
```bash
make add-to-project ISSUE_URL=https://github.com/niksmac/OpenAgentsControl/issues/123
# or
gh project item-add 2 --owner darrenhinde --url https://github.com/niksmac/OpenAgentsControl/issues/123
```
...
**Create new label:**
```bash
gh label create "priority-high" --repo niksmac/OpenAgentsControl --color "d73a4a" --description "High priority"
```
...
```bash
gh issue edit 123 --repo niksmac/OpenAgentsControl --milestone "v0.2 - DX & Examples"
```
...
for i in 123 124 125; do
  gh issue close $i --repo niksmac/OpenAgentsControl
done
```
...
```bash
for i in 123 124 125; do
  gh issue edit $i --repo niksmac/OpenAgentsControl --add-label "priority-high"
done
```
...
- **Repository:** https://github.com/niksmac/OpenAgentsControl
...
3. **Use saved searches:**
   ...
   function oa-my-issues() {
     gh issue list --repo niksmac/OpenAgentsControl --assignee @me
   }
   ```
...
4. **Combine with git workflow:**
   ...
   ISSUE=$(gh issue create --repo niksmac/OpenAgentsControl --title "Fix bug" --label bug --format json | jq -r .number)
